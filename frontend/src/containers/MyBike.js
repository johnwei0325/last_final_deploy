...
function MyBike () {
    ...
    const [parkLocationDB, setParkLocationDB] = useState("")
    const [parkLocation, setParkLocation] = useState("")
    const [parkTimeDB, setParkTimeDB] = useState(new Date());
    ...
    const handleGetMyBike = async() => {
        ...
        console.log("Handle get my Bike, username: " + username)
        console.log(myBike)
        if(myBike){
            setParked(myBike.parked);
            setParkLocationDB(myBike.parkedAt);
            setParkTimeDB(new Date(myBike.time))
        }
    ...
    const handleParkMyBike = async() => {
        const {
          data: { message, myBike },
            ...
            username, parked: !parked, parkedAt: parkLocation, time: new Date(), location: {lat: position.lat, lng: position.lng}
        });
    ...
    const handleRideMyBike = async() => {
        const {
          data: { message, myBike },
            ...
            username, parked: !parked, time: new Date(),
        });
        console.log("Handle post my Bike")
        console.log(myBike, "Is it parked? " , !parked)
        if(!myBike){
                ...
                <Typography gutterBottom variant={window.innerWidth > 410 ? "h6" : "h8"} component="div">
                    { parkLocationDB ?
                     parked? `Parked at ${parkTimeDB.toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                      ...
                      : `Start riding at : ${parkTimeDB.toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                      ${parkTimeDB.toLocaleTimeString(undefined)}`
                     : ""
                    }
                </Typography>
                ...
                <Form>
                    ...
                    onClick={async ()=> {
                        ...
                        if(!parked){
                            ...
                            setParkTimeDB(new Date())
                            setParkLocationDB(parkLocation)
                            setParked(true)
                            await handleParkMyBike(); 
                            //  handleGetMyBike({ username });
                        }else{
